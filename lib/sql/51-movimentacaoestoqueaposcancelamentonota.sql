CREATE OR REPLACE FUNCTION public.movimentacaoestoqueaposcancelamentonota() RETURNS trigger
    LANGUAGE plpgsql
    AS $body$
					DECLARE
lista record;
...
values (current_timestamp, lista.quantidade, 'E', lista.produto_id, NEW.usuariocadastro_id, estoqueAnterior, estoqueAnterior + lista.quantidade, 'Cancelamento de NF-e Nº ' || NEW.numero, 'NFE');
END IF;
END LOOP;
END IF;
					  	IF (NEW.tipoFaturamento = '0') THEN
							FOR lista IN
select inf.*                   from notafiscal.notafiscal nf
                                        inner join notafiscal.itemnotafiscal inf on  nf.numero = inf.numero
    and  nf.serie = inf.serie
    and  nf.tipofaturamento = inf.tipoFaturamento
    and   nf.empresaid = inf.empresaId
...
END IF;
INSERT INTO public.MOVIMENTOESTOQUE(datamovimentacao, quantidade, tipomovimento, produto_id, usuario_id, quantanterior, quantatual, justificativa, origemmovimentacao)
values (current_timestamp, lista.quantidade, 'S', lista.produto_id, NEW.usuariocadastro_id, estoqueAnterior, estoqueAnterior - lista.quantidade, 'Cancelamento de NF-e Nº ' || NEW.numero, 'NFE');
